...
Internal screenshot upload accepts PNG/JPEG only, validates filename suffix, MIME type, actual image format and dimensions, caps the multipart body near 10 MiB and pixels at 20 million, decodes/re-encodes pixels, and writes UUID server filenames under a private upload volume. The private Strategy Lab Nginx proxy explicitly allows request bodies up to 10 MiB only on `/api/internal/`; the backend remains responsible for validating each upload. The private upload endpoint returns 202 after durable PostgreSQL enqueue; OCR and provider validation run in the internal backend's recoverable processor, and staged status remains private. User filenames are stored as sanitized basenames for internal audit only. The public app has no upload route. Rejects do not reveal server paths. The private upload endpoint also needs authentication and edge rate limits before it is ever deployed.
