...
Universe selection is bounded at each baseline decision timestamp. A research observation proves knowledge at the later of its `observed_at` and `created_at` (so a later-persisted record cannot be backdated); a discovery membership at `first_seen_at`; an uploaded watchlist symbol at the later of `watchlist_uploads.uploaded_at` and its validated `watchlist_symbols.created_at`; and other active-universe membership at `active_universe_members.updated_at`, the persisted publication time. Missing or unrecognized source evidence is not assigned an inferred timestamp. Membership known after a decision is first eligible at the next completed 3-minute decision boundary. For example, a symbol first recorded at 09:20 CT cannot appear in the 09:18 evaluation and can first appear at 09:21. Once eligible, that symbol is evaluated through the rest of the baseline window. The ordinary date-only Strategy Lab catalog remains a full-date navigation list; baseline evaluation uses the time-bounded lookup.

## Separate historical research questions

`LIVE_RECORDED_UNIVERSE` is the authentic scanner-history baseline: it asks what the real scanner knew on each date/time and uses only persisted same-date discovery, uploaded-watchlist, active-universe, and observation evidence. Nightly catch-up continues using this mode.

`FIXED_RESEARCH_UNIVERSE` is a manually selected historical what-if experiment. The exact normalized symbol list, XNYS date range, strategy version, provider/feed metadata, timezone, decision window, source timeframe, and aggregation assumptions are persisted with the run. It evaluates every selected symbol on every requested completed XNYS session through the same historical replay service, bounded market view, 3m/10m aggregation, indicators, and frozen FORMING V1 detector. It never creates discovery memberships, uploaded watchlists, active-universe members, or scanner observations. Its symbols must not be interpreted as historically discovered or traded. This mode uses objective FORMING episode movement, direction-positive 3/5/10-bar measurements, and MFE/MAE only; entry, rejection, confirmation, stop, target, win/loss, and R-multiple logic are intentionally not implemented.

Run it locally with `python -m research.baseline --fixed-universe NVDA,AMD,TSLA,AAPL,META,AMZN,MSFT,GOOGL,AVGO,NFLX --last-trading-days 20`. Use `--start YYYY-MM-DD --end YYYY-MM-DD` instead for an explicit inclusive range. The default command without `--fixed-universe` remains the authentic live-recorded-universe baseline.

The fixed run identity includes the canonical universe and replay/provider assumptions so an identical request resumes/reuses the same experiment while a different universe or provider identity remains a separate run. Provider `NO_DATA` is recorded per symbol-day and does not stop other symbols or dates. Session selection uses the shared XNYS calendar, excludes weekends/holidays and the current incomplete session, and accepts either the previous N completed sessions or an explicit inclusive date range.

Coverage separates four facts: an XNYS trading session exists; same-date historical universe evidence is recorded; a particular symbol is known by the replay decision time; and that symbol has completed its baseline evaluation. `sessions_with_universe_coverage` counts dates with at least one evidenced candidate by the end of the window. `sessions_missing_universe` counts `NO_UNIVERSE` dates. `symbols_evaluated` counts completed symbol-days, `eligible_evaluations` counts decision-eligible candle evaluations, and `symbol_failures` counts failed symbol-days. Processing all requested dates does not make missing-universe dates research coverage: a fully processed run with any `NO_UNIVERSE` date is reported as `INCOMPLETE`, including runs created before this reporting change. CLI output labels processed sessions separately from covered/missing sessions and evaluated symbols.
